...
public class AccessibilityServiceUtils {
	...
	public static void getAllNodesWithText(List<AccessibilityNodeInfo> list,AccessibilityService service, AccessibilityNodeInfo node) {
	    if (node.getText() != null) list.add(node);
        for (int i = 0; i < node.getChildCount(); i++) {
            getAllNodesWithText(list,service, node.getChild(i));
        }
    }
